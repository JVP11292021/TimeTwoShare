...
@Component({
  ...
  template: `
    <form [formGroup]="group" (ngSubmit)="save()">
      <h1 mat-dialog-title>Dynamic Form</h1>
...
Model,
 T extends {
  service: { 
    store: (object: Model) => Observable<unknown>,
    update: (object: Model) => Observable<unknown>
  }
  ...
  public save(): void {
    this.inputData.service.update(this.inputData.build(this.group))
